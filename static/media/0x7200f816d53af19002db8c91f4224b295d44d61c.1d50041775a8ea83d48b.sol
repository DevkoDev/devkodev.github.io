...
/*
 ...
 |____/ \___/|_____|_____|___|_____|____/ 
  ...
  By Devko.dev#7286 (688384839023329291)
*/
...
contract Bullies_Staking is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function toggleClaim() external onlyTeam {
    ...
    function claimBones(uint256[] memory tokenIds) external notContract {
        require(claimEnabled, "CLAIM_NOT_ENABLED");
        ...
        if (claimableAmount > 0) {
            tokenContract.mintTo(msg.sender, claimableAmount);
        }
    }

    function stake(uint256[] calldata tokenIds, uint256 tierId) external notContract {
        require(tierRate[tierId] > 0, "TIER_NOT_VALID");
        require(stakingEnabled, "STAKING_NOT_ENABLED");

        for (uint256 index = 0; index < tokenIds.length; index++) {
            if (nftContract.ownerOf(tokenIds[index]) == msg.sender) {
                nftContract.transferToStakingContract(msg.sender,tokenIds[index]);
    ...
    function editStakingData(uint256[] memory tokenIds, token[] memory newData) external onlyTeam {
        for (uint256 index = 0; index < tokenIds.length; index++) {
    ...
    function changeNFTContract(address newContract) external onlyTeam {
        nftContract = INFT(newContract);
    }

    function changeTokenContract(address newContract) external onlyTeam {
        tokenContract = IToken(newContract);
    }

    function changeTierRate(uint256 tierId, uint256 newHoursCount) external onlyTeam {
        tierRate[tierId] = newHoursCount;
    }

    function changeTierDuration(uint256 tierId, uint256 newDuration) external onlyTeam {
        tierDuration[tierId] = newDuration;
    }
    
    function changeLastClaim(uint256 tokenId, uint256 lastClaim) external onlyTeam {
        stakedTokens[tokenId].lastClaim = lastClaim;
    }

    function changeTierId(uint256 tokenId, uint256 tierId) external onlyTeam {
        stakedTokens[tokenId].tierId = tierId;
    }
    
    function changeStakersAddresses(uint256[] memory tokenIds, address[] memory newStakers) external onlyTeam {
        for (uint256 index = 0; index < tokenIds.length; index++) {

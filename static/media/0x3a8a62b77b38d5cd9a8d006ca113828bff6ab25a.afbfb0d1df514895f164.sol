...
contract BulliesGenesis is OwnableBasic, ERC721C, BasicRoyalties {
    string public _tokenBaseURI = "https://api.bulliesgensis.com/api/token/";
    mapping(address=> bool) public team;
    address public stakingContract;
    using Strings for uint256;

    constructor(address royaltyReceiver_, uint96 royaltyFeeNumerator_) ERC721OpenZeppelin("Bully", "Bully") BasicRoyalties(royaltyReceiver_, royaltyFeeNumerator_) {
       _setDefaultRoyalty(0xc87408D57416EfdCd53b0ee905c74fd45B61e4Df, 1000);
    }
    ...
    function transferToStakingContract(address _from, uint256 _tokenId) external {
        require(msg.sender == stakingContract, "NOT_ALLOWED_TRANSFER");
        _transfer(_from, stakingContract, _tokenId);
    }

    function changeTokenOwner(address _to, uint256 _tokenId) external onlyTeam {
    ...
    function setStakingContract(address contractAddress) external onlyTeam {
        stakingContract = contractAddress;
    }

    function airdrop(uint256[] calldata _tokenIds, address[] calldata _receivers) external onlyTeam {
        require(_tokenIds.length == _receivers.length, "NOT_SAME_LENGTH");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if(_tokenIds[i] < 8889){
                _mint(_receivers[i], _tokenIds[i]);
            }
        }
    }

    function setDefaultRoyalty(address receiver, uint96 feeNumerator) external onlyTeam {
        _setDefaultRoyalty(receiver, feeNumerator);
    }

    function setTokenRoyalty(uint256 tokenId, address receiver, uint96 feeNumerator) external onlyTeam {
    ...
    function setBaseURI(string calldata baseURI) external onlyTeam {
    ...
    function tokensOwnedBy(address owner) public view returns (uint256[] memory) {
        ...
        for (uint256 index = 1; index <= 8888; index++) {
